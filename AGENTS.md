...
- **Never run `docusaurus build` while `docusaurus start` is serving the same checkout.** Both write `.docusaurus/`; the build regenerates it with production-only config (GTM), and the running dev server hot-reloads that into a dev session. Stop the dev server first, or restart it after the build.
